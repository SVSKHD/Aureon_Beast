    ...
    snapshot = build_agent_confluence(
        inputs(
            ...
            detections=(
                ...
                # A DOWN liquidity sweep means price swept a low; the implied reversal is up.
...
def test_the_meter_counts_aligned_opposed_and_neutral_separately() -> None:
    snapshot = build_agent_confluence(
        inputs(
            ema_fast=2410.0,
            ema_slow=2412.0,  # opposed to a bullish setup
            rsi=50.0,  # neutral band
            trend=TrendBias.BULLISH,
            detections=(
                detection("wick", "upper_rejection"),  # opposed
                detection("breakout", "up|session_high"),  # aligned
            ),
        ),
        DirectionContext.BULLISH,
    )
...
def test_event_driven_evidence_survives_until_that_agent_speaks_again() -> None:
    first = build_agent_confluence(
        inputs(
            detections=(detection("liquidity", "up|previous_day_high"),),
        ),
