...
| --- | --- | --- | --- | --- |
| 0 | Repo scaffolding, standing rules, boundary guards | `pytest tests/boundary` green | ✅ done | ✅ 24 boundary guards |
...
| 12 T-10 | A picture a human can disagree with | `aureon/visuals/chart_renderer.py` draws candles, the EMAs it is handed, named levels, the value area, the anchor and the invalidation line, detection and event marks and a tick-volume panel, through matplotlib's object API in a bounded worker pool; it computes no indicator, formats prices from the broker's `digits`, and turns every absence into a labelled image | ✅ 33 tests including a per-symbol golden perceptual hash and a walk of every text artist for order vocabulary | ✅ [chart renderer](../aureon/visuals/chart_renderer.py)<br>⬜ every chart so far is of fixture bars: no real session has been drawn |
| 12 T-11 | One live card per setup, edited as it advances | `aureon/discord/notifier.py` posts a card on the first announced trigger and edits the same message thereafter, with the message id on the notification document rather than on the setup (§71); the card carries the family, the direction **context**, a text state badge, the anchor, the invalidation price labelled "not a stop", the last events, the linked detection ids, the measured reference with its caption and a chart attachment; `/setups symbol:` and `/setup id:` render the same card on demand | ✅ 28 card and command tests, 14 notifier sweep tests, 26 configurable triggers with a validator | ✅ [notifier](../aureon/discord/notifier.py) · [setup view](../aureon/discord/views/setup_view.py)<br>⬜ no card has been posted to a real channel: every test injects the send and the edit |
| 12 T-12 | The recorded higher-timeframe context, verified against a replay | `scripts/compare_live_vs_replay.py --mtf` rebuilds the rolling buffer from several archived days and diffs each detection's M15/M30/H1/H4 read as of its own candle, through the same `mtf.frames_from` the observer calls; zero disagreements earns `SESSION VERIFIED (MTF)`, D1 is excluded with the reason printed, and a shallow window reports "unreadable" rather than a mismatch | ✅ 17 tests, plus a boundary test that nothing in the observation or execution path branches on an alignment value | ✅ [the comparison](../scripts/compare_live_vs_replay.py)<br>⬜ never run against a real session: no archive exists, so the marker has never been earned |
| 12 T-13/T-14 | Real history and XAG tuning stay manual | No behaviour change, by design: `tune_report.py` still has no `--apply` and cannot grow one. The no-auto-apply rule now walks every module in `aureon/` and `scripts/` for a mutation of any of the four tuning tables, including the three Phase 12 added, with the table list pinned as a literal | ✅ 2 new tests over the whole tree | ✅ [the rule](../tests/unit/test_tuning_report.py)<br>⬜ no real history has accumulated: every threshold is still a placeholder and no tuning decision has been made from measured bars |
| 13 S-1 | The storage skeleton PostgreSQL will live in | `aureon/storage/postgres/` holds the engine, the transaction boundary and the startup wait (C-4); `aureon/storage/postgres/models.py` holds the declarative base and `UtcTimestamp`, which refuses a naive timestamp in both directions; `aureon/storage/backend.py` is the seam S-4 flips, defaulting to Firestore until then and refusing an unknown value; C-9 pins the suite to a database named `aureon_test` and refuses `aureon` by name; C-10 finds a server through `AUREON_DATABASE_URL` or `AUREON_TEST_ADMIN_URL` and skips without one | ✅ 38 unit tests, 13 integration tests against a real server, 2 new boundary tests, 37 plants caught | ✅ [the plan](POSTGRES_MIGRATION_PLAN.md) · [database](../aureon/storage/postgres/database.py)<br>⬜ no table exists yet: the schema is S-2 and no service reads PostgreSQL until S-4 |
| 13 S-2 | The schema, in one revision | `aureon/storage/postgres/tables.py` holds all 24 tables under plan §7's column rule; `0001_initial` is generated from it and REFUSES to downgrade, because dropping the four money tables would be indistinguishable from the trades never having happened (C-7); detections keep `candle_close_utc`, `candle_time_utc`, `timezone` and `market_date` (C-6); `scripts/migrate.py upgrade|current|check` fails in BOTH directions and separates "unreachable" from "wrong schema" by exit code; the preflight `migrations` row SKIPs until S-4 switches the backend; `docs/CONTRACTS.md` gains a tables section generated from the metadata | ✅ 15 unit tests, 25 integration tests against a real server, contracts `--check` clean | ✅ [tables](../aureon/storage/postgres/tables.py) · [0001_initial](../aureon/storage/postgres/migrations/versions/0001_initial.py)<br>⬜ no repository writes a row yet (S-3) and no service reads one (S-4); every table is empty |
| 13 S-3a | The money path's repositories, with the atomicity actually proved | detections, detection_evaluations, setups + setup_events, audit_logs, trade_requests, control_requests, trades and notifications on PostgreSQL. §12's setup transaction and §15's executor claim keep their logic and lose the Firestore caveat that their unit tests proved no isolation (decision 350); C-1 adds `RECONCILING` with transitions from EXECUTING only; C-2's `sl`/`tp` are copied through and a structural test forbids the money path assigning either; §58's terminal-write rule and §71's audit-in-transaction rule both carried over | ✅ 168 integration tests against a real server, 5 unit tests for RECONCILING, 42 plants caught — 5 of them locking plants that an earlier thread-based race test could not see (decision 352) | ✅ [trade_requests](../aureon/storage/postgres/repositories/trade_requests.py) · [contention](../tests/postgres/contention.py)<br>⬜ no service uses these yet (S-4); no order has been sent through them, on demo or otherwise |
| 13 S-3b | The rest of the tables, and two races a row lock could not stop | heartbeats, system_state, settings, symbol_specs, ops_events, alerts, assessments, trade_notes, setup_evaluations, daily/weekly reviews, sessions and the broker-day cache on PostgreSQL, each keeping the method names its Firestore counterpart had so S-4 is a constructor swap (decision 360); the kill switch writes its row and its audit record in one transaction under an advisory lock, and so does the per-user alert cap, because `FOR UPDATE` protects neither a row that does not exist yet nor a COUNT (decision 359); `sessions` gains the `timezone` column a `MarketTime` cannot be rebuilt without (decision 357) and `ops_events.detail` becomes the string it always was (decision 358); C-13's `history_source` filter is a WHERE clause, so a readout measured over generated bars cannot reach a real-history report | ✅ 170 integration tests against a real server, 47 plants caught — two of them races that every non-overlapped test passed | ✅ [operations](../aureon/storage/postgres/repositories/operations.py) · [alerts](../aureon/storage/postgres/repositories/alerts.py)<br>⬜ no service uses these yet (S-4); `sync_batches` waits for its model in S-9 (decision 361) |
